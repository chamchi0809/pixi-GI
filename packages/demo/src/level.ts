...
export function buildLevel(tex: LevelTextures): Level {
    for (const row of MAP) {
        if (row.length !== COLS) throw new Error(`level rows must be ${COLS} chars, got ${row.length}`);
    }
    ...
    const solid: boolean[][] = MAP.map((row) => [...row].map((c) => SOLID.has(c)));
    let spawnX = TILE * 2;
    let spawnY = TILE * 2;

    const place = (parent: Container, texture: Texture, col: number, row: number, scale = 1): Sprite => {
    ...
    for (let row = 0; row < ROWS; row++) {
        ...
        for (let col = 0; col < COLS; col++) {
            ...
            switch (cell) {
                ...
                case 'S':
                    setMaterial(place(solidLayer, tex.stoneCenter, col, row), { occlusion: 1 });
                    break;
                case '#':
                    setMaterial(place(solidLayer, tex.grassMid, col, row), { occlusion: 1 });
                    break;
                case '=':
                    setMaterial(place(solidLayer, tex.grassCenter, col, row), { occlusion: 1 });
                    break;
                case 'x':
